...
const jobPostingArbitrary = fc.record({
  ...
  salaryRange: fc.option(fc.string({ minLength: 1, maxLength: 50 }), { nil: null }),
  location: fc.string({ minLength: 1, maxLength: 100 }),
  status: fc.constantFrom(JobPostingStatus.OPEN, JobPostingStatus.CLOSED, JobPostingStatus.DRAFT)
});
...
const applicationArbitrary = fc.record({
  ...
  aiSummary: fc.option(fc.string({ minLength: 1, maxLength: 500 }), { nil: null })
});

const interviewArbitrary = fc.record({
  type: fc.constantFrom(InterviewType.SCREENING, InterviewType.TECHNICAL, InterviewType.FINAL),
  scheduledAt: fc.date({ min: new Date('2020-01-01'), max: new Date('2030-12-31') }),
...
const evaluationArbitrary = fc.record({
  ...
  feedback: fc.option(fc.string({ minLength: 1, maxLength: 500 }), { nil: null })
});

const onboardingArbitrary = fc.record({
  tasks: fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }), fc.anything()),
  status: fc.constantFrom(
...
async function createApplicationWithRelations(
  ...
  const savedCandidate = await candidateRepo.save(candidate) as unknown as Candidate;

  const jobPosting = jobPostingRepo.create(jobPostingData);
  const savedJobPosting = await jobPostingRepo.save(jobPosting) as unknown as JobPosting;
  ...
  const savedApplication = await applicationRepo.save(application) as unknown as Application;

  const interviews = interviewsData.map(data =>
    interviewRepo.create({ ...data, application: savedApplication })
  ) as unknown as Interview[];
  const savedInterviews = interviews.length > 0 ? await interviewRepo.save(interviews) : [] as Interview[];

  const evaluations = evaluationsData.map(data =>
    evaluationRepo.create({ ...data, application: savedApplication })
  ) as unknown as Evaluation[];
  const savedEvaluations = evaluations.length > 0 ? await evaluationRepo.save(evaluations) : [] as Evaluation[];
...
async function verifyRelatedRecordsExist(
  ...
  if (interviewIds.length > 0) {
    const count = await interviewRepo.countBy({ application: { id: applicationId } });
    expect(count).toBe(interviewIds.length);
  }

  if (evaluationIds.length > 0) {
    const count = await evaluationRepo.countBy({ application: { id: applicationId } });
...
async function verifyRelatedRecordsDeleted(applicationId: string, onboardingId: string | null) {
  const interviewRepo = testDataSource.getRepository(Interview);
  const evaluationRepo = testDataSource.getRepository(Evaluation);
  const onboardingRepo = testDataSource.getRepository(Onboarding);

  const interviewsAfter = await interviewRepo.countBy({ application: { id: applicationId } });
  expect(interviewsAfter).toBe(0);

  const evaluationsAfter = await evaluationRepo.countBy({ application: { id: applicationId } });
  expect(evaluationsAfter).toBe(0);

  if (onboardingId) {
    const onboardingAfter = await onboardingRepo.findOneBy({ id: onboardingId });
...
async function cleanupEntities(
  ...
  if (savedApplication) {
    const app = await applicationRepo.findOneBy({ id: savedApplication.id });
...
describe('Entity Property-Based Tests', () => {
  ...
  describe('Property 1: Audit Column Creation Timestamp', () => {
    it('should automatically populate createdAt when entity is saved', async () => {
      await fc.assert(
        fc.asyncProperty(candidateArbitrary, async (candidateData) => {
          ...
          expect(savedCandidate.createdAt.getTime()).toBeGreaterThanOrEqual(beforeSave.getTime() - 1000);
          expect(savedCandidate.createdAt.getTime()).toBeLessThanOrEqual(afterSave.getTime() + 1000);
    ...
    it('should automatically populate createdAt for JobPosting entities', async () => {
      await fc.assert(
        fc.asyncProperty(jobPostingArbitrary, async (jobPostingData) => {
          const jobPostingRepo = testDataSource.getRepository(JobPosting);
          ...
          expect(savedJobPosting.createdAt.getTime()).toBeGreaterThanOrEqual(beforeSave.getTime() - 1000);
          expect(savedJobPosting.createdAt.getTime()).toBeLessThanOrEqual(afterSave.getTime() + 1000);
  ...
  describe('Property 2: Audit Column Update Timestamp', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (candidateData, newLocation) => {
            ...
            const updatedCandidate = await candidateRepo.save(savedCandidate);
            ...
            expect(updatedCandidate.updatedAt.getTime()).toBeGreaterThanOrEqual(initialUpdatedAt.getTime());
            
            // Verify updatedAt is within reasonable time range of update
            expect(updatedCandidate.updatedAt.getTime()).toBeGreaterThanOrEqual(beforeUpdate.getTime() - 1000);
            expect(updatedCandidate.updatedAt.getTime()).toBeLessThanOrEqual(afterUpdate.getTime() + 1000);
    ...
    it('should automatically update updatedAt for JobPosting entities', async () => {
      await fc.assert(
        fc.asyncProperty(
          jobPostingArbitrary,
          fc.constantFrom(JobPostingStatus.OPEN, JobPostingStatus.CLOSED, JobPostingStatus.DRAFT),
          async (jobPostingData, newStatus) => {
            ...
            const beforeUpdate = new Date();
            const updatedJobPosting = await jobPostingRepo.save(savedJobPosting);
            const afterUpdate = new Date();
            
            // Verify updatedAt changed
            expect(updatedJobPosting.updatedAt).toBeDefined();
            expect(updatedJobPosting.updatedAt).toBeInstanceOf(Date);
            expect(updatedJobPosting.updatedAt.getTime()).toBeGreaterThanOrEqual(initialUpdatedAt.getTime());
            expect(updatedJobPosting.updatedAt.getTime()).toBeGreaterThanOrEqual(beforeUpdate.getTime() - 1000);
            expect(updatedJobPosting.updatedAt.getTime()).toBeLessThanOrEqual(afterUpdate.getTime() + 1000);
    ...
    it('should cascade delete all related Applications when Candidate is deleted', async () => {
      await fc.assert(
        fc.asyncProperty(
          candidateArbitrary,
          jobPostingArbitrary,
          fc.array(applicationArbitrary, { minLength: 1, maxLength: 5 }),
          async (candidateData, jobPostingData, applicationsData) => {
            ...
            const savedApplications = await applicationRepo.save(applications);
            const applicationIds = savedApplications.map(app => app.id);
    ...
    it('should cascade delete all related Interview, Evaluation, and Onboarding records when Application is deleted', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.array(interviewArbitrary, { minLength: 0, maxLength: 3 }),
          fc.array(evaluationArbitrary, { minLength: 0, maxLength: 3 }),
          ...
          async (
            ...
            try {
              ...
              await verifyRelatedRecordsDeleted(savedApplication.id, result.onboardingId);
              ...
              const jobPostingRepo = testDataSource.getRepository(JobPosting);
              try {
                await cleanupEntities(savedApplication, savedCandidate, savedJobPosting);
              } catch (cleanupError) {
